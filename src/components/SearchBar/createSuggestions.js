  ...
  await fetch(`https://api.hel.fi/servicemap/v2/search/?input=${query}&only=unit.name&page_size=${pageSize}&type=unit`)
    ...
    .then((data) => {
      ...
      data.results.forEach((result) => {
        const resultWords = getLocaleText(result.name).split(' ');
    ...
    .then(async (searchWord) => {
      ...
      const data = await fetch(`https://api.hel.fi/servicemap/v2/search/?input=${searchWord}&only=unit.id&page_size=${pageSize}&type=unit`)
        .then(response => response.json());
      if (data && data.count) {
      ...
      Object.keys(serviceMap).forEach((key) => {
        if (Object.prototype.hasOwnProperty.call(serviceMap, key)) {
          ...
          const test = Promise.all([
            ...
            fetch(`https://api.hel.fi/servicemap/v2/search/?page=1&page_size=1&only=unit.id&q=${query}&${current.type === 'root' ? 'service_node' : 'service'}=${key}&type=unit`).then(res => res.json()),
          ]);
...
const createSuggestions = async (query, getLocaleText) => {
  ...
  await Promise.all([
    fetch(`https://api.hel.fi/servicemap/v2/search/?input=${query}&only=unit.location,unit.name,unit.municipality,unit.accessibility_shortcoming_count&page_size=3&type=unit`)
      .then(response => response.json())
      .then((data) => {
